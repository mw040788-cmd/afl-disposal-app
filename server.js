...
const ROLE_HINTS = [
  { names: ["nick daicos","josh daicos","christian salem","callum mills","john noble","ed richards","justin mcinerney","jack sinclair","dayne zorko","harry sheezel","callum mills"], role: "outside", comp: "low", blowout: "good" },
  { names: ["marcus bontempelli","lachie neale","patrick cripps","george hewett","touk miller","zach merrett","josh dunkley","tim kelly","harley reid","andrew brayshaw","caleb serong","jack steele","noah anderson","matthew kennedy"], role: "inside", comp: "high", blowout: "mixed" },
  { names: ["kysaiah pickett","joel freijah","archie roberts"], role: "hybrid", comp: "medium", blowout: "mixed" },
  { names: ["will ashcroft","ryley sanders","jason horne-francis","finn callaghan","luke davies-uniacke"], role: "mid-hybrid", comp: "high", blowout: "mixed" }
];
...
function roleGuess(name) {
  ...
  for (const hint of ROLE_HINTS) {
    if (hint.names.some(x => n.includes(x))) return { role: hint.role, comp: hint.comp, blowout: hint.blowout };
  }
...
function autoGuessFromText(text) {
  ...
  let matchup = "";
  let venue = "";
  if (teams.length >= 2) matchup = `${titleCase(teams[0])} vs ${titleCase(teams[1])}`;
...
function normalizeText(text) {
  return (text || "")
    .replace(/Player Disposals Over\/Under/ig, "\n")
    .replace(/Last 5:/ig, " ")
    .replace(/[|]/g, " ")
    .replace(/\u00A0/g, " ")
    .replace(/\r/g, "\n")
...
function extractCandidates(text) {
  ...
  for (const line of lines) {
    ...
    if (m && isLikelyName(m[1])) candidates.push({ source: "line-name-line", name: m[1].trim(), line: parseFloat(m[2]) });

    m = line.match(/^([A-Z][A-Za-z'.-]+(?:\s+[A-Z][A-Za-z'.-]+){1,3}).*?\b(\d{2}\.\d)\b/);
    if (m && isLikelyName(m[1])) candidates.push({ source: "line-loose", name: m[1].trim(), line: parseFloat(m[2]) });
  }

  // Global scan patterns
  const patterns = [
    /([A-Z][A-Za-z'.-]+(?:\s+[A-Z][A-Za-z'.-]+){1,3})\s+(\d{2}\.\d)\b/g,
    /([A-Z][A-Za-z'.-]+(?:\s+[A-Z][A-Za-z'.-]+){1,3})[\s\S]{0,16}?(\d{2}\.\d)\b/g,
    /([A-Z][A-Za-z'.-]+(?:\s+[A-Z][A-Za-z'.-]+){1,3})[\s\S]{0,32}?Player Disposals Over\/Under[\s\S]{0,24}?(\d{2}\.\d)\b/gi
  ];

  for (const regex of patterns) {
    let m;
    while ((m = regex.exec(text)) !== null) {
      if (isLikelyName(m[1])) candidates.push({ source: "global", name: m[1].trim(), line: parseFloat(m[2]) });
    }
  }

  // Dedup and score frequency
  const freq = new Map();
...
function autoScript(matchup, manual) {
  ...
  if (teams.length >= 2) {
    const a = TEAM_DB[teams[0]], b = TEAM_DB[teams[1]];
...
function scorePlayers(payload) {
  ...
  const results = (payload.players || []).map(p => {
    ...
    const total = lineHardness + recent + roleScore + compScore
    ...
    return { ...p, total, decision: total >= 1 ? "MORE" : "LESS", confidence: classifyConfidence(Math.abs(total)) };
...
app.post("/api/parse-text", (req, res) => {
  ...
  res.json({ ok: true, rawText: text, guessed, candidates, players });
});

app.post("/api/score", (req, res) => {
  res.json({ ok: true, ...scorePlayers(req.body || {}) });
});
...
app.post("/api/parse-screenshot", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    fs.unlink(req.file.path, () => {});
    res.status(500).json({ ok: false, error: "OCR failed.", detail: String(err) });
  }
});

app.listen(PORT, () => {
  console.log(`AFL V6 starter running on http://localhost:${PORT}`);
});
