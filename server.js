...
const ROLE_HINTS = [
  { names: ["nick daicos","josh daicos","christian salem","callum mills","john noble","ed richards","justin mcinerney","jack sinclair","dayne zorko","harry sheezel","bailey smith","max holmes"], role: "outside", comp: "low", blowout: "good" },
  { names: ["marcus bontempelli","lachie neale","patrick cripps","george hewett","touk miller","zach merrett","josh dunkley","tim kelly","harley reid","andrew brayshaw","caleb serong","jack steele","noah anderson","matthew kennedy","jai newcombe","james sicily"], role: "inside", comp: "high", blowout: "mixed" },
  { names: ["kysaiah pickett","joel freijah","archie roberts","tanner bruhn"], role: "hybrid", comp: "medium", blowout: "mixed" },
...
function extractCandidates(text) {
  ...
  for (const line of lines) {
    ...
    if (m && isLikelyName(m[1])) candidates.push({ source: "line-start", name: m[1].trim(), line: parseFloat(m[2]) });
...
app.listen(PORT, () => {
  console.log(`AFL V7 starter running on http://localhost:${PORT}`);
});
